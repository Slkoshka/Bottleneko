...
class Response {
    ...
    async blob() {
        return Blob._blobFromStream(await this._response.ReadAsMemoryStreamAsync(), { type: this.headers.get('content-type') });
    }
    async bytes() {
    ...
    async formData() {
        ...
        for (let i = 0; i < items.Count; i++) {
            ...
            if (typeof item.Data === 'string') {
                ...
                formData.append(item.Name, File._fileFromStream(item.Data, item.Filename, { type: item.ContentType }), item.Filename);
            }
        }
    }
    async json() {
...
export async function fetch(resource, options) {
    ...
    let bodyStream = null;
    let guessedContentType = null;
    if (body !== null) {
        if (body instanceof FormData) {
            guessedContentType = 'multipart/form-data';
            bodyStream = body._toNativeStream();
        }
            ...
            bodyStream = body._nativeStream;
        }
    }
    const response = await __Api.HttpClient.MakeRequestAsync(request.method, request.url, bodyStream, request.headers.get('content-type') ?? guessedContentType, request.headers._asObject(), request.redirect === 'follow');
